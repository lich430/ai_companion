...
def classify_user_message(text: str) -> dict:
    ...
    low_context = (
        len(compact) <= 1
        or only_marks
        or (len(compact) <= 3 and compact not in LOW_CONTEXT_EXEMPT)
        or (len(alpha_num) <= 3 and any(ch.isalpha() for ch in alpha_num))
    )
    hostile = any(k in lowered for k in HOSTILE_KEYWORDS)
    boring = compact in BORING_EXACT or (len(compact) <= 2 and not hostile and not low_context)
